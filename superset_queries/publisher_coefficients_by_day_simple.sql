-- Упрощенный отчет: Коэффициенты паблишеров с разбивкой по дням
-- БЕЗ параметров - используйте фильтры Dashboard для выбора периода
-- 
-- Для использования:
-- 1. Создайте Chart на основе этого запроса
-- 2. Добавьте фильтры на Dashboard (Date Range Filter для date)
-- 3. Примените фильтры к Chart через Scoping

WITH 
-- 1. Получаем данные о расходах (фильтры применяются на уровне Dashboard)
daily_spend AS (
    SELECT 
        publisher_id,
        publisher_name,
    ...
    WHERE publisher_id != 0  -- Исключаем органику
      AND spend >= 50  -- Минимальные расходы
),
