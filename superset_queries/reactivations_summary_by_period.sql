...
-- Агрегированная статистика реактиваций за период
-- 
...
-- 5. Подсчитываем новых пользователей
new_users AS (
    SELECT COUNT(DISTINCT user_id) as new_count
    FROM reactivations_with_period
    WHERE prev_deposit_date IS NULL
)

-- 6. Итоговая сводка по периодам неактивности
SELECT
    r.inactivity_period,
